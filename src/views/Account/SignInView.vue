<template>
  ...
  >
    <div class="mb-8 text-center">
      <h1 class="my-1 text-3xl font-bold lowercase">Вход</h1>
    </div>
    <form @submit.prevent="submit" class="space-y-4 ng-untouched ng-pristine ng-valid">
      ...
      >
        ...
        <div class="flex justify-end text-xs dark:text-gray-400">
          <RouterLink rel="noopener noreferrer" to="/forgotpassword">
            Забыли пароль?
          </RouterLink>
        </div>
      </b-itext>
...
const submit = async () => {
  ...
  if (!success) {
    ...
    backenderrors.value.push("Неверная эл.почта или пароль");
    loader.value = false;
    return;
  }
  await router.push("/");
